...
SELECT 
	week_start,
	week_end,
	COALESCE(max(n) FILTER (WHERE district = 'central'), 0) AS C,
	COALESCE(max(n) FILTER (WHERE district = 'eastern'), 0) AS E,
	COALESCE(max(n) FILTER (WHERE district = 'metrosouth'), 0) AS MS,
	COALESCE(max(n) FILTER (WHERE district = 'northeast'), 0) AS NE,
	COALESCE(max(n) FILTER (WHERE district = 'southeast'), 0) AS SE,
	COALESCE(max(n) FILTER (WHERE district = 'western'), 0) AS W
FROM (
